...
class LoginViewModel: Stepper, ViewModelProtocol {
    ...
    init(_ loginService: LoginServiceProtocol) {
        ...
        signInDidTapSubject.subscribe(onNext: { [weak self] event in
            self?.steps.accept(MorniStep.registerIsRequired)
            })
